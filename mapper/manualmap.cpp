...
uintptr_t injection::manualmap::find_or_map_dependecy(const std::string& image_name)
{
	...
	for (auto module : this->mapped_modules)
		if (module.image_name == image_name)
			return std::get<uintptr_t>(module.remote_image);
	...
	if (map_image(ctx))
		return std::get<uintptr_t>(ctx.remote_image);
...
{
	memcpy(std::get<void*>(ctx.local_image), ctx.get_pe_buffer(), ctx.pe.get_optional_header().SizeOfHeaders);
}
void injection::manualmap::write_image_sections(map_ctx& ctx)
{
	for (auto section : ctx.pe.get_sections())
		memcpy(reinterpret_cast<void*>(std::get<uintptr_t>(ctx.local_image) + section.VirtualAddress), ctx.get_pe_buffer() + section.PointerToRawData, section.SizeOfRawData);
}
...
void injection::manualmap::call_entrypoint(map_ctx& ctx)
{
	...
	*PPTR(shellcode + 0x6) = std::get<uintptr_t>(ctx.remote_image);
	*PPTR(shellcode + 0x1A) = std::get<uintptr_t>(ctx.remote_image) + ctx.pe.get_optional_header().AddressOfEntryPoint;
...
{
	auto delta = std::get<uintptr_t>(ctx.remote_image) - ctx.pe.get_image_base();

	for (auto&[entry, item] : ctx.pe.get_relocations(std::get<uintptr_t>(ctx.local_image)))
...
void injection::manualmap::fix_import_table(map_ctx& ctx)
{
	...
	for (auto&[tmp_name, functions] : ctx.pe.get_imports(std::get<uintptr_t>(ctx.local_image)))
	{
